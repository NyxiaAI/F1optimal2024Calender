...
cities = [
    ...
    (26.0325, 50.5100, "Asia", "Sakhir", "no"),
    (21.4225, 39.8262, "Asia", "Jeddah", "no"),
    (34.8431, 136.5419, "Asia", "Suzuka", "no"),
    (31.2001, 121.6057, "Asia", "Shanghai", "harbour"),
    (25.3436, 51.4367, "Asia", "Lusail", "no"),
    (24.4672, 54.6031, "Asia", "Yas Marina", "no"),
    (40.3723, 49.8532, "Asia", "Baku", "no"),
        ...
        if from_city_name != to_city_name:
            ...
            for mode in ["Plane", "Train", "Ship"]:
                ...
                if (mode == "Train" or mode == "Truck") and from_city_continent != to_city_continent:
                    # at continent level: allow trains between Asia and Europe only
                    # otherwise, allow only trains and trucks within the same continent
                    if not (mode == "Train" and (from_city_continent == "Asia" or to_city_continent == "Asia")
                            and (from_city_continent == "Europe" or to_city_continent == "Europe")):
                        continue
            ...
            if len(visited_continent) == len(continents_to_cities):
                ...
                for edge in continents_to_cities[from_continent][end_city].edges:
                    if edge.end.continent not in visited_continent:
                        whole_trip(edge.end.continent, edge.end.name, visited_continent + [edge.end.continent], route2 + [
                            "BETWEEN:", end_city, edge.end.name], emission2 + edge.emission, duration2 + edge.time,
                            edges2 + [edge], distance2 + edge.distance, time_in_transport2 + edge.time_in_transport)
